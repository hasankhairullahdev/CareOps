...
import { CreditCard, Plus, Pencil, Trash2, CheckCircle2, XCircle, X } from "lucide-react";
...
export default function PaymentMethodsPage() {
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      const data = await paymentMethodsApi.list(session.accessToken as string) as PaymentMethod[];
      setItems(data);
  ...
  function openEdit(t: PaymentMethod) {
    setForm({ name: t.name, description: t.description ?? "", isActive: t.isActive });
    setFormError(null);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (modal.editing) await paymentMethodsApi.update(session.accessToken as string, modal.editing.id, payload);
      else await paymentMethodsApi.create(session.accessToken as string, payload);
      closeModal(); load();
  ...
  async function handleDelete(id: string, name: string) {
    ...
    setDeleting(id);
    try {
      await paymentMethodsApi.delete(session!.accessToken as string, id);
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Manage accepted payment methods</p>
          </div>
        </div>
      ...
      {loading ? <div className="text-sm text-gray-400 py-10 text-center">Loading…</div> : (
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.length === 0 && <tr><td colSpan={4} className="text-center py-10 text-gray-400">No payment methods yet.</td></tr>}
              {items.map(p => (
                <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => openEdit(p)} className="p-1.5 text-gray-400 hover:text-primary hover:bg-primary/10 rounded-lg transition-colors"><Pencil size={13} /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-bold text-gray-900">{modal.editing ? "Edit Method" : "Add Method"}</h2>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                ...
                <input required value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Cash"
